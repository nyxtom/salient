...
Word.prototype.collapseLinks = function (mapping) {
    ...
    else if (bigram && this.checkStopTerm(mapping, bigram)) {
        this.mergeLeft(mapping);
        return true;
    }
    ...
    else if (trigram && this.checkStopTerm(mapping, trigram)) {
        this.mergeLeft(mapping);
        this.mergeLeft(mapping);
        return true;
    }
    ...
    else if (quadrigram && this.checkStopTerm(mapping, quadrigram)) {
        this.mergeLeft(mapping);
        this.mergeLeft(mapping);
        this.mergeLeft(mapping);
        return true;
...
Word.prototype.checkTerms = function (mapping, term, inspectRight, takeLeft) {
    ...
    this.isFiltered = this.isStopTerm || this.isAdpNonScorable;
};
