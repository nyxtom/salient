...
Word.prototype.filter = function (mapping) {
    ...
    var filters = this.checkFiltered(mapping);
    if (filters > -1) {
...
Word.prototype.collapse = function (mapping) {
    // check linkings to collapse
    var times = this.checkMap(mapping, 'link');
    if (times > 0) {
    ...
    times = this.checkMap(mapping, 'cop');
    if (times > 0) {
        for (var i = 0; i < times; i++) {
            this.mergeLeft(mapping);
        }
...
Word.prototype.checkMap = function (mapping, prop) {
    ...
    if (this.next && this.next.next && this.next.next.next && mapping[prop][3].hasOwnProperty(this.term)) {
        if (mapping[prop][3][this.term].hasOwnProperty(this.next.term)) {
            if (mapping[prop][3][this.term][this.next.term].hasOwnProperty(this.next.next.term)) {
                if (mapping[prop][3][this.term][this.next.term][this.next.next.term].hasOwnProperty(this.next.next.next.term)) {
    ...
    // check trigram
    if (this.next && this.next.next && mapping[prop][2].hasOwnProperty(this.term)) {
        if (mapping[prop][2][this.term].hasOwnProperty(this.next.term)) {
            if (mapping[prop][2][this.term][this.next.term].hasOwnProperty(this.next.next.term)) {
    ...
    // check bigram
    if (this.next && mapping[prop][1].hasOwnProperty(this.term)) {
        if (mapping[prop][1][this.term].hasOwnProperty(this.next.term)) {
            // successful stop-term found in trigram
            return 1;
        }
    }
    // check unigram
    if (mapping[prop][0].hasOwnProperty(this.term)) {
        // successful stop-term found in trigram
        return 0;
    }
...
Word.prototype._updateState = function (mapping) {
    ...
    this.isAdpNonScorable = this.hasOwnProperty('isAdpNonScorable') ? this.isAdpNonScorable : mapping['adp0'].indexOf(lowerTerm) > -1;
