...
function HiddenMarkovModel(options) {
    ...
    this.tags = {'0': 'NOUN', '1': 'VERB', '2': 'ADJ', '3': 'ADV', '4': 'X', '5': 'NUM', 
                 '6': 'PRON', '7': 'PRT', '8': 'CONJ', '9': 'DET', '10': 'ADP', '11': '.', '*': '*'
                };
    this.tagLength = 12;
};
...
HiddenMarkovModel.prototype.loadVocab = function (vocabFile, tagCount, callback) {
    ...
    rl.on('line', function (line) {
        ...
        var posItems = items[2].split(',').sort();
        for (var p = 0; p < posItems.length; p++) {
        ...
        var pos = item.pos;
        var equivalenceClass = pos.sort().join(',');
...
HiddenMarkovModel.prototype.viterbi = function (tokens) {
    ...
    for (var k = 0; k < tokens.length; k++) {
        ...
        var index = this.tagClasses.length - 1;
        var index = this.vocabulary.get(token);

        if (!index) {
            index = this.tagClasses.length - 1;
        }
        else {
            index = this.data.pos[index];
        }
        ...
        var tagProbK = this.vocabularyTagProbability[index];
        var stateK = this.tagClasses[index];
        var tags = [];
        ...
        for (var uIndex = 0; uIndex < statesU.length; uIndex++) {
            ...
            for (var vIndex = 0; vIndex < statesV.length; vIndex++) {
                ...
                for (var wIndex = 0; wIndex < statesW.length; wIndex++) {
                    ...
                    var transitionQ = trigramDist.p;
                    var prob = prevPi[w][u] * transitionQ * (emissionV);
    ...
    for (var u in prevPi) {
        for (var v in prevPi[u]) {
            ...
            if (this.tagGramDistribution.hasOwnProperty(stopGram)) {
                distProb = this.tagGramDistribution[stopGram].p;
            }
