...
function BrownCorpus(file, options) {
    ...
    this.output = this.options.output || path.join(__dirname, 'en-brown.tag.dist')
};
...
BrownCorpus.prototype.loadFiles = function (files) {
    ...
    for (var i = 0; i < sortedTags.length; i++) {
        ...
        fs.appendFile(this.output, line);
    }
    var endTime = new Date().getTime();
    console.log(util.format('Wrote %s distributions from %s to %s (in %sms)', sortedTags.length, sentences.length, 
