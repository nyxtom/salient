...
function PennTreeBank(file, options) {
    ...
    this.fileName = file;
    this.mapping = Mapping.load(path.join(__dirname, '/universal_tagset/en-ptb.map'));
    this.output = this.options.output || path.join(__dirname, 'en-ptb.tag.vocab')
    this.outputDist = this.options.output || path.join(__dirname, 'en-ptb.tag.dist')
    this.outputSentences = this.options.outputSentences || path.join(__dirname, 'en-ptb.sentences');
};

PennTreeBank.prototype.parse = function () {
    var self = this;
...
PennTreeBank.prototype.suffix = function (token, length, minLength) {
    if (token.indexOf('\'') >= 0) {
...
PennTreeBank.prototype.loadFiles = function (files) {
    var self = this;
    var sentences = [];
    var tags = {};
    var vocab = {};
...
PennTreeBank.prototype.dist = function (tags) {
    var d = [];
    for (var t in tags) {
        var item = tags[t];
        item.t = t;
...
PennTreeBank.prototype.storeState = function (tags, sentences, state) {
...
PennTreeBank.prototype.addStateTag = function (state, tag) {
...
PennTreeBank.prototype.updateState = function (state, word, tag) {
    if (word) {
...
PennTreeBank.prototype.addVocabularyTag = function (vocab, word, tag) {
...
PennTreeBank.prototype.parseLine = function (line) {
    // [ token/POS token/POS ]
    // token/POS
    // token/POS token/POS
