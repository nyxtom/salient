...
Glossary.prototype.parse = function (text, ignoreChunking) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (this.lem) {
            ...
            if (items.length > 0) {
                ...
                if (w.lemma == w.term || (w.distinct && w.distinct == w.lemma)) {
                    delete w.lemma;
                }
            }
        }
    ...
    var lemmaTime = new Date().getTime();
    this.lemmaTime += (lemmaTime - newEndTime);

    this.filter();
    this.collapse(false);
    if (!ignoreChunking)
        this.collapse(true);
    this.collapseTime += (new Date().getTime() - lemmaTime);
};
