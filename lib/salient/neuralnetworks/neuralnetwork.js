...
NeuralNetwork.prototype.backpropagate = function (ThetaLD, X, y, lambda) {
    ...
    for (var l = layers - 1; l >= 0; l--) {
        ...
        // Remove the bias unit to perform sigmoid gradient
        weightedErrorL = weightedErrorL.slice(1, weightedErrorL.rows(), 2, weightedErrorL.cols());
        ...
        deltaOutput = weightedErrorL.elementMultiply(weightedInputMinus1.sigmoidGradient());
    }
    ...
    for (var i = 0; i < ThetaGrad.length; i++) {
        ...
        var adjust = lambda / m;
        ThetaGrad[i] = gradAugment.multiply(adjust);
    }
