...
function DocumentGraph(options) {
    ...
    this._prefixes = [total,content,weight];
};
...
DocumentGraph.prototype._incrEdges = function (id, key, prevKey) {
    ...
    this._redisClient.incrby(this._fmt(TOTAL, TERMS), 1);
    ...
    if (prevKey) {
        this._redisClient.zincrby(this._fmt(CONTEXT, prevKey), 1, key);
        this._redisClient.zincrby(this._fmt(CONTEXT, key), 1, prevKey);
    }
...
DocumentGraph.prototype.indexWeights = function (id, callback) {
    var self = this;
    this._redisClient.zrevrange(this._fmt(id), 0, -1, function (err, results) {
        if (!results) {
            callback(false);
            return;
        }
...
DocumentGraph.prototype._cosineFilter = function (prefix, filterPrefixes, id1, id2, callback) {
    ...
    var id2_scores = {};
    var ids = [];
    self._redisClient.zrevrange(self._fmt(prefix, id1), 0, -1, 'WITHSCORES', function (err, id1_results) {
        if (err) {
            callback(err, null);
            return;
        }

        self._redisClient.zrevrange(self._fmt(prefix, id2), 0, -1, 'WITHSCORES', function (err, id2_results) {
            if (err) {
                callback(err, null);
                return;
            }
            ...
            while (id1_results.length) {
                ...
                if (prefix == CONTEXT && self._totalTerms) {
...
DocumentGraph.prototype.CosineConceptSimilarity = function (id1, id2, callback) {
    this._cosineFilter(WEIGHT, ['noun','adj'], id1, id2, callback);
};
...
DocumentGraph.prototype.CosineContextSimilarity = function (term1, term2, callback) {
    if (!this._totalTerms) {
        var self = this;
        this._redisClient.get(this._fmt(TOTAL, TERMS), function (err, result) {
            if (err) {
                callback(err, null);
                return;
            }
            self._totalTerms = result;
            self._cosineFilter(CONTEXT, null, term1, term2, callback);
        });
    } else {
        this._cosineFilter(CONTEXT, null, term1, term2, callback);
    }
};
