...
DocumentGraph.prototype.search = function (terms, callback) {
    var self = this;
    async.map(terms, function (term, cb) {
        ...
        async.map(terms, function (term, cb) {
            ...
            // if term begins with tag:, then we can compute the tfidf
            // over all the relevant documents on this term
            if (term.indexOf(':') > 0) {
                self._redisClient.zrange(self._fmt(term), 0, self.options.searchLimit, function (err, results) {
                    async.map(results, function (id, cb2) {
                        self.TFIDF(id, term, function (tferr, tfidfResult) {
                            if (tferr) {
                                cb2(err);
                            } else {
                                cb2(null, [id, tfidfResult.tfidf]);
                            }
                        });
                    }, function (err, tfidfResults) {
                        if (err) {
                            cb(err);
                        } else {
                            var t = [];
                            for (var i = 0; i < tfidfResults.length; i++) {
                                t.push(tfidfResults[i][0]);
                                t.push(tfidfResults[i][1]);
                            }
                            cb(null, t);
                        }
                    });
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                while (termResults.length) {
                    ...
                    var docTermWeight = termResults.shift();
