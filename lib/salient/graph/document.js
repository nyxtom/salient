...
function DocumentGraph(options) {
    ...
    if (this.options.redisCluster) {
        ...
        for (var i = 0; i < addresses.length; i++) {
            ...
            var host = items[0];
            var port = items[1];
            config.push({ port: parseInt(port), host: host });
        }
        this._redisCluster = RedisCluster.create(config);
    } else {
...
DocumentGraph.prototype._client = function (key) {
    return this._redisClient || this._redisCluster.getRedis(key);
};
...
DocumentGraph.prototype.search = function (terms, callback, options) {
    ...
    async.map(terms, function (term, cb) {
        ...
        var selectResults = function () {
            self._client(outKey).zcard(outKey, function (err, cardResult) {
                if (err) {
                    callback(err);
                } else {
                    self._client(outKey).zrevrange(outKey, 0, options.searchLimit, 'WITHSCORES', function (err, results) {
                        if (err) {
                            callback(err);
                            return;
                        }
        ...
        self._client(outKey).exists(outKey, function (err, existsResult) {
            if (err) {
                callback(err);
                ...
                if (self._redisCluster) {
                    ...
                    async.map(clients, function (item, cb) {
                        item[0].zcard(item[1], function (err, reply) { cb(err, [item[1], reply]); });
                    }, function (err, results) {
                        if (err) {
                            callback(err);
                        } else {
                            results = results.sort(function (a, b) { return b[1] - a[1] });
                            var primaryClient = self._client(results[0][0]);
                            var delKeys = [];
                    ...
                    self._client(args[0]).zunionstore(args, function (err, result) {
                        if (err) {
                            callback(err);
                        } else {
                            selectResults();
        ...
        this._redisCluster.execMany('get', formattedIds, function (err, results) {
            if (err) {
                ...
                while (formattedIds.length) {
                    r.push(results[formattedIds.shift()]);
                    r.push(results[formattedIds.shift()]);
                    r.push(results[formattedIds.shift()]);
                }
                callback(null, r);
...
DocumentGraph.prototype.readDocument = function (id, text, title, link, categories) {
    ...
    if (title) {
        parseLoop(this, title, 10, categoryWeights);
    }
    if (text) {
...
DocumentGraph.prototype.indexWeights = function (id, callback) {
    var self = this;
    var k_DocTermId = this._fmt(DOC_TERM, id);
    this._client(k_DocTermId).zrevrange(k_DocTermStar, 0, -1, function (err, keys) {
        if (!results) {
            callback(false);
            return;
        }
...
DocumentGraph.prototype.TFIDF = function (id, key, callback) {
    ...
    if (this._redisCluster) {
        var keys = [this._k_Total, totalDocKey, this._fmt(TERM_DOC, key)];
        if (id) {
        ...
        async.map(maps, function (item, clusterCallback) {
            if (item.length == 2) {
                item[0](item[1], clusterCallback);
            } else {
                item[0](item[1], item[2], clusterCallback);
            }
        }, execResults);
    }
    else if (id) {
        this._redisClient.multi()
            .get(this._k_Total).get(totalDocKey).zcard(this._fmt(TERM_DOC, key)).zscore(this._fmt(DOC_TERM, id), key)
            .exec(execResults);
    } else {
        this._redisClient.multi()
            .get(this._k_Total).get(totalDocKey).zcard(this._fmt(TERM_DOC, key))
            .exec(execResults);
    }
};
