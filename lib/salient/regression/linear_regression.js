...
LinearRegression.prototype.train = function () {
    ...
    var result = this.gradientDescent(theta, X, y, alpha, iterations);
    return result;
};

/**
 * Standard implementation of the Multivariant Square-Error Cost function 
 * using the implementation of J(Θ) = (XΘ-y)'(XΘ-y) / 2m
 *
 ...
 */
LinearRegression.prototype.computeCost = function (theta, X, y) {
    var m = X.rows();
    var result = X.multiply(theta).subtract(y);
    return result.transpose().multiply(result).div(2 * m).elements[0][0];
};
 ...
 */
LinearRegression.prototype.gradientDescent = function (theta, X, y, alpha, iterations) {
    var m = X.rows();
    var history = [];
    ...
    for (var i = 0; i < iterations; i++) {
        ...
        history.push(this.computeCost(theta, X, y));
    }
